...
export function FavoriteButton({ itineraryId }: { itineraryId: string }) {
    const { user, isFavorite, addFavorite, removeFavorite, loading } = useAuth();
    const { toast } = useToast();
    const isFav = isFavorite('itinerary', itineraryId);
    ...
    const handleToggleFavorite = async (e: React.MouseEvent) => {
        ...
        if (isFav) {
            await removeFavorite('itinerary', itineraryId);
            toast({ title: "Eliminado de favoritos" });
        } else {
            await addFavorite('itinerary', itineraryId);
