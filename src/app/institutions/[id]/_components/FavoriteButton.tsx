...
export function FavoriteButton({ institutionId }: { institutionId: string }) {
    const { user, isFavorite, addFavorite, removeFavorite, loading } = useAuth();
    const { toast } = useToast();
    const isFav = isFavorite('institution', institutionId);
    ...
    const handleToggleFavorite = async (e: React.MouseEvent) => {
        ...
        if (isFav) {
            await removeFavorite('institution', institutionId);
            toast({ title: "Eliminado de favoritos" });
        } else {
            await addFavorite('institution', institutionId);
