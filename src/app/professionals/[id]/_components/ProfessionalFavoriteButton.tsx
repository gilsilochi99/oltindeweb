...
export function ProfessionalFavoriteButton({ professionalId }: { professionalId: string }) {
    const { user, isFavorite, addFavorite, removeFavorite, loading } = useAuth();
    const { toast } = useToast();
    const isFav = isFavorite('professional', professionalId);
    ...
    const handleToggleFavorite = async (e: React.MouseEvent) => {
        ...
        if (isFav) {
            await removeFavorite('professional', professionalId);
            toast({ title: "Eliminado de favoritos" });
        } else {
            await addFavorite('professional', professionalId);
