...
export async function generateStaticParams() {
    ...
    return facilities.filter((f) => f.type === 'pharmacy').map((facility) => ({
      id: facility.id,
    }));
}
...
export default async function PharmacyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <HealthFacilityDetailView facility={facility} services={services} detailPath="/health/pharmacies" />;
}
